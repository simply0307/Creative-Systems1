...
const assignArtifactFolder = async (supabase, { artifact, artifactId, folderPath, profile, updateReason = "indexed" }) => {
  ...
  const provenance = {
    ...
    indexedPath,
    [`${updateReason}InArchiveIndexAt`]: new Date().toISOString(),
  };
  const legacy_data = {
    ...(before.legacy_data || {}),
    folder: normalizedFolderPath,
...
const handleUpdateControlledValue = async ({ request, supabase, identity, profile, kind, valueId }) => {
  requireRole(identity, "admin");
  const body = await bodyJson(request);
  const table = kind === "categories" ? "categories" : kind === "tags" ? "tags" : null;
  if (!table) throw Object.assign(new Error("Controlled value type must be categories or tags."), { status: 404 });
...
const handleRepoMetadataImport = async ({ supabase, identity, profile }) => {
  requireRole(identity, "admin");
...
const handleArchiveFolderImport = async ({ supabase, identity, profile }) => {
  requireRole(identity, "admin");
...
const handleMoveArtifact = async ({ request, supabase, identity, profile, artifactId }) => {
  requireRole(identity, "contributor");
  const body = await bodyJson(request);
...
const handleCreateBrowserImportBatch = async ({ request, supabase, identity, profile }) => {
  requireRole(identity, "contributor");
  const body = await bodyJson(request);
  ...
  const batch = requireData(await supabase.from("import_batches").insert({ id, title: String(body.title || `Browser file import ${new Date().toISOString().slice(0, 10)}`), source: "browser-multi-file", status: "in_progress", created_by: profile.id, manifest }).select().single(), "Create browser import batch");
...
const handleImportBatchFileStatus = async ({ request, supabase, identity, profile, batchId }) => {
  requireRole(identity, "contributor");
  ...
  const entry = { clientFileId: key, fileName: safeFileName(body.fileName), relativePath: String(body.relativePath || ""), fileSize: Number(body.fileSize || 0), status, artifactId: body.artifactId || null, matchedArtifactId: body.matchedArtifactId || null, matchedBy: body.matchedBy || null, error: body.error || null, updatedAt: new Date().toISOString() };
...
const handleCompleteBrowserImportBatch = async ({ supabase, identity, profile, batchId }) => {
  requireRole(identity, "contributor");
  const batch = await loadAuthorizedBatch(supabase, batchId, identity, profile);
  const manifest = cleanObject(batch.manifest);
  const files = Array.isArray(manifest.files) ? manifest.files : [];
  ...
  requireData(await supabase.from("import_batches").update({ status, manifest: { ...manifest, completedAt: new Date().toISOString() } }).eq("id", batchId), "Complete browser import batch");
...
const handleUploadSign = async ({ request, supabase, config, identity }) => {
  requireRole(identity, "contributor");
  const body = await bodyJson(request);
  ...
  const date = new Date();
  const path = `${identity.userId}/${date.getUTCFullYear()}/${String(date.getUTCMonth() + 1).padStart(2, "0")}/${checksumSha256 ? `${checksumSha256.slice(0, 12)}-` : ""}${randomUUID()}-${fileName}`;
...
const handleUploadComplete = async ({ request, supabase, config, identity, profile }) => {
  requireRole(identity, "contributor");
  const body = await bodyJson(request);
  ...
  const id = match.artifact?.id || `artifact.${slugBase}-${randomUUID().slice(0, 8)}`;
  ...
  const provenance = { ...(existing?.provenance || {}), ...cleanObject(body.provenance), workspaceRelativePath: body.relativePath || existing?.provenance?.workspaceRelativePath || existing?.legacy_data?.filePath || null, folder: uploadFolderPath, checksumSha256: body.checksumSha256 || existing?.provenance?.checksumSha256 || null, uploadedAt: new Date().toISOString(), importBatchId: body.importBatchId || null };
  const row = existing ? {
  ...
  try {
    ...
    if (appliesImmediately) {
      folderAssignment = await assignArtifactFolder(supabase, { artifactId: id, folderPath: uploadFolderPath, profile, updateReason: "uploaded" });
    }
  } catch (error) {
    await supabase.storage.from(config.artifactsBucket).remove([storagePath]);
    throw error;
...
const handleCreateExport = async ({ request, supabase, config, identity, profile }) => {
  requireRole(identity, "contributor");
  const body = await bodyJson(request);
  ...
  const payload = { schemaVersion: "1.0", generatedAt: new Date().toISOString(), exportType, generatedBy: profile.email, artifacts };
  ...
  const id = randomUUID();
  const path = `${new Date().toISOString().slice(0, 10)}/${id}-${slugify(exportType)}.json`;
...
export const handleCreativeOsRequest = async (request, context = {}, overrides = {}) => {
  ...
  try {
    ...
    if (request.method === "PATCH" && controlledValueMatch) return await handleUpdateControlledValue({ request, supabase, identity, profile, kind: controlledValueMatch[1], valueId: controlledValueMatch[2] });
    ...
    if (request.method === "POST" && path === "imports/repo-metadata") return await handleRepoMetadataImport({ supabase, identity, profile });
    if (request.method === "POST" && path === "imports/archive-folder") return await handleArchiveFolderImport({ supabase, identity, profile });
    if (request.method === "POST" && path === "folders") return await handleCreateFolder({ request, supabase, identity, profile });
    if (request.method === "POST" && path === "import-batches") return await handleCreateBrowserImportBatch({ request, supabase, identity, profile });
    const importFileMatch = path.match(/^import-batches\/([0-9a-f-]+)\/files$/i);
    if (request.method === "PATCH" && importFileMatch) return await handleImportBatchFileStatus({ request, supabase, identity, profile, batchId: importFileMatch[1] });
    const importCompleteMatch = path.match(/^import-batches\/([0-9a-f-]+)\/complete$/i);
    if (request.method === "POST" && importCompleteMatch) return await handleCompleteBrowserImportBatch({ supabase, identity, profile, batchId: importCompleteMatch[1] });
    if (request.method === "POST" && path === "uploads/sign") return await handleUploadSign({ request, supabase, config, identity, profile });
    if (request.method === "POST" && path === "uploads/complete") return await handleUploadComplete({ request, supabase, config, identity, profile });
    ...
    if (request.method === "POST" && moveMatch) return await handleMoveArtifact({ request, supabase, identity, profile, artifactId: decodeURIComponent(moveMatch[1]) });
    ...
    if (request.method === "POST" && path === "exports") return await handleCreateExport({ request, supabase, config, identity, profile });
...
export default handleCreativeOsRequest;
