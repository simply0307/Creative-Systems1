...
const runRoleOperation = async (role, payload, { autoApprove="true", merge=true } = {}) => {
  ...
  Object.assign(process.env,{GITHUB_TOKEN:"token",GITHUB_OWNER:"simply0307",GITHUB_REPO:"creative-systems1",GITHUB_DEFAULT_BRANCH:"master",GITHUB_AUTHOR_NAME:"Creative OS",GITHUB_AUTHOR_EMAIL:"os@example.test",ADMIN_AUTO_APPROVE:autoApprove});
...
test("invalid GitHub response shape fails visibly", async () => {
  const adapter = new GitHubAdapter({ env:{ GITHUB_TOKEN:"bad", GITHUB_OWNER:"simply0307", GITHUB_REPO:"creative-systems1", GITHUB_DEFAULT_BRANCH:"master", GITHUB_AUTHOR_NAME:"OS", GITHUB_AUTHOR_EMAIL:"os@example.test" }, fetchImpl:async()=>new Response("{}", { status:200 }) });
