...
const runRoleOperation = async (role, payload, { autoApprove="true", merge=true } = {}) => {
  ...
  Object.assign(process.env,{GITHUB_TOKEN:"token",GITHUB_OWNER:"simply0307",GITHUB_REPO:"creative-systems",GITHUB_DEFAULT_BRANCH:"master",GITHUB_AUTHOR_NAME:"Creative OS",GITHUB_AUTHOR_EMAIL:"os@example.test",ADMIN_AUTO_APPROVE:autoApprove});
  globalThis.fetch=async(url,options={})=>{const method=options.method||"GET";let body;if(url.includes("/git/ref/heads/master"))body={object:{sha:"base"}};else if(url.includes("/git/commits/base"))body={tree:{sha:"tree-base"}};else if(url.includes("/contents/src/content/artifacts/sample.json"))body={sha:"artifact",content:Buffer.from(JSON.stringify({id:"artifact.sample",title:"Sample",tags:[],relatedProjects:[]})).toString("base64")};else if(url.endsWith("/git/blobs"))body={sha:`blob-${++blobCount}`};else if(url.endsWith("/git/trees"))body={sha:`tree-${blobCount}`};else if(url.endsWith("/git/commits")&&method==="POST")body={sha:`commit-${++commitCount}`};else if(url.endsWith("/pulls")){pulls.push(JSON.parse(options.body));body={html_url:"https://github.com/simply0307/creative-systems/pull/9",number:9,draft:pulls.at(-1).draft};}else if(url.endsWith("/pulls/9/merge"))body={merged:merge,sha:merge?"merge-sha":null,message:merge?undefined:"Branch protection"};else body={};return new Response(JSON.stringify(body),{status:200});};
...
test("decision write plan includes resolution, rewrite request, and change log", async () => {
  ...
  const finalized = plan.finalizeFiles("https://github.com/eggs/creative-os/pull/42");
...
test("invalid GitHub response shape fails visibly", async () => {
  const adapter = new GitHubAdapter({ env:{ GITHUB_TOKEN:"bad", GITHUB_OWNER:"simply0307", GITHUB_REPO:"creative-systems", GITHUB_DEFAULT_BRANCH:"master", GITHUB_AUTHOR_NAME:"OS", GITHUB_AUTHOR_EMAIL:"os@example.test" }, fetchImpl:async()=>new Response("{}", { status:200 }) });
...
test("invalid admin key prevents every GitHub write attempt", async () => {
  ...
  try {
    ...
    assert.match(body.fallbackReason, /Sign in/);
...
test("operations health endpoint reports configuration without secrets", async () => {
  ...
  try {
    ...
    const body = await response.json();
    assert.equal(body.defaultBranch, "master");
    assert.equal(body.diagnostics.apiReached, true);
...
test("GitHub adapter stages a branch, commits files, and opens a draft PR", async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    let body;
    if (url.includes("/git/ref/heads/master")) body = { object: { sha: "base-commit" } };
    ...
    else if (url.endsWith("/pulls")) body = { html_url: "https://github.com/eggs/creative-os/pull/42", number: 42, draft: true };
    else body = {};
    return new Response(JSON.stringify(body), { status: 200, headers: { "content-type": "application/json" } });
  };
  const adapter = new GitHubAdapter({
    env: {
      GITHUB_TOKEN: "secret-token", GITHUB_OWNER: "eggs", GITHUB_REPO: "creative-os",
      GITHUB_DEFAULT_BRANCH: "master", GITHUB_AUTHOR_NAME: "Creative OS", GITHUB_AUTHOR_EMAIL: "os@example.test",
    },
    fetchImpl,
  });
  ...
  assert.equal(result.pullRequestUrl, "https://github.com/eggs/creative-os/pull/42");
...
test("unauthenticated viewer cannot write and receives a clear error", async()=>{const response=await operations({httpMethod:"POST",headers:{},body:JSON.stringify({action:"metadata.update",actor:"Unknown",targets:["artifact.sample"],changes:{addTags:["motif"]}})});const body=JSON.parse(response.body);assert.equal(response.statusCode,401);assert.equal(body.authenticated,false);assert.match(body.fallbackReason,/Sign in/);});
...
test("Admin Portal moves successful database actions immediately before queue refresh",async()=>{const source=await readFile(new URL("../src/pages/admin.astro",import.meta.url),"utf8");assert.match(source,/item\.status=response\.reviewStatus\|\|item\.status;status\.textContent=response\.message;render\(\);setTimeout\(load/);assert.match(source,/data-action="approve">Approve & apply/);assert.match(source,/data-action="reject">Reject/);});
...
test("Workbench detail page includes a Resolution Panel",async()=>{const source=await readFile(new URL("../src/pages/workbench/[id].astro",import.meta.url),"utf8");assert.match(source,/Resolution Panel/);assert.match(source,/Submit resolution/);assert.match(source,/canonStatusResult/);});
...
test("artifact tag UI persists proposals, reports database failures, and refreshes live tags",async()=>{const client=await readFile(new URL("../src/scripts/creative-os-client.js",import.meta.url),"utf8"),page=await readFile(new URL("../src/pages/pipeline/artifacts.astro",import.meta.url),"utf8");assert.match(client,/api\/creative-os/);assert.doesNotMatch(client,/fetch\("\/api\/operations"/);assert.match(page,/Pending tag review/);assert.match(page,/Database write failed/);assert.match(page,/operation-error/);assert.match(page,/replaceArtifact\(result\.artifact\)/);assert.match(page,/artifact\.signedUrl/);});
...
test("Artifact Library controls are non-overlapping and bulk editor is collapsible",async()=>{const css=await readFile(new URL("../src/styles/global.css",import.meta.url),"utf8"),page=await readFile(new URL("../src/pages/pipeline/artifacts.astro",import.meta.url),"utf8");assert.match(css,/\.artifact-toolbar\s*\{\s*position:static/);assert.match(css,/\.bulk-editor\s*\{[^}]*position:static/s);assert.match(page,/<details class="bulk-shell">/);});
